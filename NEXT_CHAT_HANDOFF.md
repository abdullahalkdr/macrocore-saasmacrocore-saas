...
12. **موقع macrocore التسويقي** (`marketing/`) — مشروع منفصل.
...
## 6. وضع الكود الحالي بـ git (مهم جداً — لسا ما تُرفع)

كل التعديلات بالقسم 5 فوق (من البند 7 لين 18 تقريباً، بالإضافة لتعديلات على ملفات موجودة) **موجودة محلياً بس فقط، ما انرفعت GitHub ولا بالتالي ما انبنت على الاستضافة (Railway)**. تأكدنا بـ `git status`:
- عندك تعديلات كثيرة على ملفات موجودة (`modified`) بالباك-إند والفرونت-إند.
- عندك ملفات جديدة كاملة غير متتبّعة (`Untracked`) — منها كل الميقريشنز 010–015، `backend/nodemon.json`، كنترولرز/راوتس جديدة (`inventory`, `apiKeys`, `customFields`, `documentTemplates`)، صفحة `InventoryOverviewPage.tsx`، `themeStore.ts`، مجلد `frontend/src/pages/account/` كامل، **ومجلد `marketing/` كامل**.

**يعني: أي عميل تجريبي يدخل الموقع المباشر الحين بيشوف النسخة القديمة** (قبل كل هالشغل)، مو الداشبورد والرواتب اللي اشتغلنا عليها.

### كيف ترفعها (git push) عشان يشوفها العميل التجريبي

من نفس نافذة الباك-إند أو نافذة جديدة:
...
راجع القائمة اللي تطلع — تأكد ما فيه ملفات حساسة (زي `.env` الحقيقي بمفاتيح/باسوردات حقيقية) قبل ما تكمل. إذا طلعت `.env` بالقائمة **وقف ولا تكمل** وقولي.

```powershell
git commit -m "لوحة التحكم: الداشبورد + الرواتب + المخزون + إصلاحات"
git push origin main
```

إذا Railway مربوط بحساب GitHub هذا بـ auto-deploy (الوضع المعتاد)، بيبدأ يبني وينشر تلقائي بعد الـ push مباشرة — تابع تقدم البناء من لوحة تحكم Railway نفسها (مو من هني، ما عندي وصول لحسابك). خذ له عادة دقايق لين يخلص البناء والنشر.

**بعد ما يخلص النشر — لا تنسى**: تشغّل الميقريشنز 013/014/015 (القسم 4 فوق) على قاعدة Railway الحقيقية، وإلا الصفحات الجديدة تطلع أخطاء أو أرقام فاضية حتى بعد نشر الكود.
